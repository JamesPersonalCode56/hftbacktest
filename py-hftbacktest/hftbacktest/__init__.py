...
class BacktestAsset(BacktestAsset_):
    ...
    def initial_snapshot(self, data: str | EVENT_ARRAY):
        """
        ...
        if isinstance(data, str):  
            ...
            if not os.path.exists(data):  
                raise FileNotFoundError(f"Initial snapshot file not found: {data}")  
            super().initial_snapshot(data)  
        elif isinstance(data, np.ndarray):  
            self._initial_snapshot_ndarray(data.ctypes.data, len(data))  
        else:  
            raise ValueError("data must be either a file path string or NumPy array")  
        return self
